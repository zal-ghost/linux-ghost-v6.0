...
static int detach_tasks(struct lb_env *env)
{
	 ...
	 */
	if (rq_adj_nr_running(env->src_rq) <= 1) {
		env->flags &= ~LBF_ALL_PINNED;
		return 0;
	}
...
static int should_we_balance(struct lb_env *env)
{
	 ...
	 */
	if (env->idle == CPU_NEWLY_IDLE) {
		if (rq_adj_nr_running(env->dst_rq) > 0 || env->dst_rq->ttwu_pending)
			return 0;
		return 1;
	}
...
redo:
	...
	if (rq_adj_nr_running(busiest) > 1) {
		 ...
		 */
		env.loop_max  = min(sysctl_sched_nr_migrate, rq_adj_nr_running(busiest));
